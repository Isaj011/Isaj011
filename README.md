<!-- ============================================================
  BATMAN: ARKHAM KNIGHT GITHUB PROFILE — Isaj011 / Ismael Saad
  Interactive: https://Isaj011.github.io
  ============================================================ -->

<div align="center">

<!-- ══════════════════════════════════════════════════════════
     HERO BANNER — THE BIG CALL TO ACTION
     This is the first thing visitors see on your profile
════════════════════════════════════════════════════════════ -->

<br/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Orbitron&weight=700&size=20&duration=3000&pause=800&color=3A7BD5&center=true&width=700&lines=BATCOMPUTER+ONLINE...;ARKHAM+SYSTEMS+v4.7;LEAD+FULL-STACK+ENGINEER+%7C+NAIROBI+KE;MICROFRONTENDS+%C2%B7+SPRING+BOOT+%C2%B7+DJANGO;BUILDING+SYSTEMS+THAT+WORK+IN+THE+DARK)](https://Isaj011.github.io)

<br/>

```
╔══════════════════════════════════════════════════════════════════════════╗
║                                                                          ║
║           ██████╗  █████╗ ████████╗███╗   ███╗ █████╗ ███╗              ║
║           ██╔══██╗██╔══██╗╚══██╔══╝████╗ ████║██╔══██╗████╗             ║
║           ██████╔╝███████║   ██║   ██╔████╔██║███████║██╔██╗            ║
║                                                                          ║
║          ISAJ011 / README.md  ·  ARKHAM SYSTEMS v4.7                    ║
║          OPERATIVE: ISMAEL SAAD  ·  NAIROBI, KE → REMOTE               ║
║                                                                          ║
╚══════════════════════════════════════════════════════════════════════════╝
```

<br/>

<!-- ── THE MAIN BUTTON — MAKE IT BIG AND UNMISSABLE ── -->

[![ENTER ARKHAM SYSTEMS](https://img.shields.io/badge/%E2%96%BA%20%20ENTER%20FULL%20INTERACTIVE%20PROFILE%20%20%E2%96%BA-04060d?style=for-the-badge&logoColor=3a7bd5&labelColor=04060d&color=04060d)](https://Isaj011.github.io)

[![LIVE TERMINAL](https://img.shields.io/badge/LIVE%20TERMINAL-04060d?style=flat-square&logoColor=3a7bd5&color=04060d)](https://Isaj011.github.io)&nbsp;[![BATCOMPUTER](https://img.shields.io/badge/BATCOMPUTER-04060d?style=flat-square&color=04060d)](https://Isaj011.github.io)&nbsp;[![PLAYABLE GAME](https://img.shields.io/badge/PLAYABLE%20GAME-04060d?style=flat-square&color=04060d)](https://Isaj011.github.io)&nbsp;[![GOTHAM RAIN](https://img.shields.io/badge/GOTHAM%20RAIN-04060d?style=flat-square&color=04060d)](https://Isaj011.github.io)

<sub><sup>

```
──────────────────────────────────────────────────────────────────────────
  Click ▲ above for the full Dark Knight experience — Isaj011.github.io
  Live terminal  ·  Batcomputer PC  ·  Commit Runner game  ·  Gotham rain
──────────────────────────────────────────────────────────────────────────
```

</sup></sub>

<br/>

</div>

---

<div align="center">

```
╔══════════════════════════════════════════════════════════════════════╗
║  ARKHAM SYSTEMS v4.7  ·  OPERATIVE: ISAJ011  ·  THREAT: NOMINAL     ║
║  BASE: NAIROBI, KE → REMOTE  ·  CLEARANCE: LEVEL 5  ·  ● ONLINE    ║
╚══════════════════════════════════════════════════════════════════════╝
```

</div>

---

### `// IDENTITY FILE`

```
CODENAME  : ISAJ011
NAME      : Ismael Saad
ROLE      : Lead Frontend & Full-Stack Engineer
CURRENT   : Serianu Limited — Nairobi, Kenya
BASE      : Nairobi, KE → Remote
CLEARANCE : Open Source · Open to Contracts
MISSION   : Microfrontend architecture. Enterprise systems.
            Fintech innovation. Distributed at scale.
            Clean code. Ship fast. Zero bugs.
```

---

### `// FIELD STATISTICS`

<div align="center">

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Isaj011&show_icons=true&count_private=true&theme=dark&hide_border=true&bg_color=04060d&title_color=3a7bd5&text_color=a8c8f0&icon_color=5a9ff5&ring_color=3a7bd5)](https://github.com/Isaj011)

[![Streak](https://streak-stats.demolab.com?user=Isaj011&theme=dark&hide_border=true&background=04060d&ring=3a7bd5&fire=5a9ff5&currStreakLabel=3a7bd5&sideLabels=a8c8f0&dates=3a5880&currStreakNum=d0e8ff&sideNums=d0e8ff)](https://github.com/Isaj011)

[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Isaj011&layout=compact&langs_count=8&theme=dark&hide_border=true&bg_color=04060d&title_color=3a7bd5&text_color=a8c8f0)](https://github.com/Isaj011)

</div>

<!-- STATS:START -->
```
REPOS     : 30    FOLLOWERS : 47    STARS : 38    UPDATED : 2026-05-02
```
<!-- STATS:END -->

---

### `// UTILITY BELT — TECH ARSENAL`

<div align="center">

![React](https://img.shields.io/badge/React.js-04060d?style=for-the-badge&logo=react&logoColor=5a9ff5)
![Next.js](https://img.shields.io/badge/Next.js-04060d?style=for-the-badge&logo=next.js&logoColor=a8c8f0)
![TypeScript](https://img.shields.io/badge/TypeScript-04060d?style=for-the-badge&logo=typescript&logoColor=5a9ff5)
![JavaScript](https://img.shields.io/badge/JavaScript-04060d?style=for-the-badge&logo=javascript&logoColor=3a7bd5)
![Vite](https://img.shields.io/badge/Vite-04060d?style=for-the-badge&logo=vite&logoColor=c8a84b)
![Redux](https://img.shields.io/badge/Redux_Toolkit-04060d?style=for-the-badge&logo=redux&logoColor=9b59b6)
![Tailwind](https://img.shields.io/badge/Tailwind-04060d?style=for-the-badge&logo=tailwindcss&logoColor=5a9ff5)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-04060d?style=for-the-badge&logo=springboot&logoColor=2ecc71)
![Django](https://img.shields.io/badge/Django-04060d?style=for-the-badge&logo=django&logoColor=2ecc71)
![Node.js](https://img.shields.io/badge/Node.js-04060d?style=for-the-badge&logo=node.js&logoColor=3a7bd5)
![Express](https://img.shields.io/badge/Express.js-04060d?style=for-the-badge&logo=express&logoColor=a8c8f0)
![GraphQL](https://img.shields.io/badge/GraphQL-04060d?style=for-the-badge&logo=graphql&logoColor=c8a84b)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-04060d?style=for-the-badge&logo=postgresql&logoColor=3a7bd5)
![MongoDB](https://img.shields.io/badge/MongoDB-04060d?style=for-the-badge&logo=mongodb&logoColor=2ecc71)
![MySQL](https://img.shields.io/badge/MySQL-04060d?style=for-the-badge&logo=mysql&logoColor=5a9ff5)
![Redis](https://img.shields.io/badge/Redis-04060d?style=for-the-badge&logo=redis&logoColor=c0392b)
![Docker](https://img.shields.io/badge/Docker-04060d?style=for-the-badge&logo=docker&logoColor=3a7bd5)
![Kubernetes](https://img.shields.io/badge/Kubernetes-04060d?style=for-the-badge&logo=kubernetes&logoColor=5a9ff5)
![AWS](https://img.shields.io/badge/AWS-04060d?style=for-the-badge&logo=amazonaws&logoColor=c8a84b)
![GitHub Actions](https://img.shields.io/badge/CI%2FCD-04060d?style=for-the-badge&logo=githubactions&logoColor=a8c8f0)
![Figma](https://img.shields.io/badge/Figma-04060d?style=for-the-badge&logo=figma&logoColor=c0392b)

</div>

---

### `// OPERATIVE ASSESSMENT`

```
Frontend Systems    ████████████░  93%   React · Next.js · TypeScript · Vite · RTK
Spring Boot / Java  ███████████░░  88%   REST APIs · JWT · RBAC · Enterprise grade
Django / Python     ███████████░░  87%   Reporting Engine · PDF Gen · ReportLab
Microfrontend Arch  ███████████░░  88%   Module Federation · Independent Deploys
API Architecture    ███████████░░  88%   REST · GraphQL · WebSockets · API Gateway
Database Intel      ██████████░░░  82%   PostgreSQL · MongoDB · Schema Optimisation
DevOps & Cloud      █████████░░░░  72%   Docker · Kubernetes · AWS / Azure / GCP
Technical Lead      ████████████░  90%   Code Review · Mentorship · Architecture
```

---

### `// CASE FILES — ACTIVE OPERATIONS`

<table>
<tr>
<td width="33%" valign="top">

**`CASE #001`** &nbsp; 🟢 **LIVE**

**Vehicle & Route Management**

Full-stack logistics platform for Kenya's transport ecosystem. JWT, RBAC, microservices-ready.

`Node.js` `Express` `MongoDB` `React`

[Repo ↗](https://github.com/Isaj011/vehicle-route-management)

</td>
<td width="33%" valign="top">

**`CASE #002`** &nbsp; 🟢 **LIVE**

**Django Reporting Engine**

Enterprise PDF/Docx generation from JSON-driven templates. Deployed at Serianu Limited.

`Django` `Python` `ReportLab` `PostgreSQL`

[Repo ↗](https://github.com/Isaj011/django-reporting-engine)

</td>
<td width="33%" valign="top">

**`CASE #003`** &nbsp; 🟢 **LIVE**

**3D Developer Portfolio**

Interactive Three.js portfolio with optimised rendering. Deployed on Netlify.

`React` `Three.js` `Vite`

[Live ↗](https://3d-app-portfolio-saad.netlify.app) &nbsp; [Classic ↗](https://myfolio-isaj011-nyatirogue.netlify.app)

</td>
</tr>
</table>

---

### `// CURRENT MISSION — SERIANU LIMITED`

```
ORGANISATION : Serianu Limited  |  Nairobi, Kenya
ROLE         : Lead Frontend & Full-Stack Engineer
TENURE       : May 2023 – Present

KEY OPS      :
  ► Led migration to Microfrontend architecture
  ► Built Spring Boot (Java) + Django (Python) backend services
  ► Architected Django Reporting Engine — dynamic PDF generation
  ► Designed PostgreSQL & MongoDB schemas optimised for scale
  ► Implemented JWT + RBAC across distributed systems
  ► Mentored engineers · led architecture reviews · Agile delivery
```

---

### `// 52-WEEK SURVEILLANCE LOG`

<div align="center">

![Snake](https://github.com/Isaj011/Isaj011/blob/output/github-contribution-grid-snake-dark.svg)

</div>

---

### `// OPEN CHANNEL`

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-04060d?style=for-the-badge&logo=github&logoColor=a8c8f0)](https://github.com/Isaj011)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-04060d?style=for-the-badge&logo=linkedin&logoColor=3a7bd5)](https://linkedin.com/in/isaj011)
[![Portfolio 3D](https://img.shields.io/badge/Portfolio_3D-04060d?style=for-the-badge&logo=vercel&logoColor=a8c8f0)](https://3d-app-portfolio-saad.netlify.app)
[![Portfolio Classic](https://img.shields.io/badge/Portfolio_Classic-04060d?style=for-the-badge&logo=netlify&logoColor=5a9ff5)](https://myfolio-isaj011-nyatirogue.netlify.app)

</div>

---

<div align="center">

```
[ ARKHAM SYSTEMS · CLASSIFIED · AGENT STATUS: OPERATIONAL ]
"It's not who I am underneath — but what I build that defines me."
```

![Visitors](https://profile-counter.glitch.me/Isaj011/count.svg)

</div>
